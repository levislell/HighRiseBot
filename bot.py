...
# Dictionnaire d'emotes - Partie 1
EMOTES = {
    ...
    "knocscreen": "emote-knocking-screen",
    "curtsy": "emote-curtsy",
    "slap": "emote-slap",
    "twerk": "dance-twerk",
    ...
    "think": "emote-think",
    "headbobbing": "idle-dance-headbobbing",
    ...
    "loophappy": "idle-loop-happy",
    "heartfingers": "emote-heartfingers",
    "collabphoto": "emote-collab-photo-left"
}
...
class Bot(BaseBot):
    ...
    async def on_chat(self, user: User, message: str):
        ...
        # EMOTES EN MOT BRUT (ex: "voguehands", "heartfingers", "griddy")
        if text in EMOTES:
            try:
                await self.highrise.send_emote(user.id, EMOTES[text])
            except Exception:
                pass
            return
        ...
        if text == "!help":
            await self.highrise.chat("📜 Tapez simplement le nom d'une danse (ex: voguehands, griddy, heartfingers) ou utilisez !dance ou !wave")
            ...
            try:
                await self.highrise.send_emote(user.id, "idle-dance-casual")
            except Exception:
                pass

        elif text == "!wave":
            try:
                await self.highrise.send_emote(user.id, "idle-wave")
            except Exception:
                pass

        elif text.startswith("!emote"):
            if user.id not in OWNERS:
                await self.highrise.chat("❌ Commande réservée au propriétaire.")
                return

            emote = message[6:].strip()
            try:
                await self.highrise.send_emote(user.id, emote)
            except Exception:
                await self.highrise.chat("❌ Emote inconnue.")
